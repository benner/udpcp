...
# Detect container runtime (podman preferred, docker fallback; override via CONTAINER_RUNTIME).
...
build_static_binary() {
    ...
    cargo build --release --target "$MUSL_TARGET" \
        --manifest-path "$SCRIPT_DIR/Cargo.toml" --quiet ||
        die "cargo build failed"
    cp "$SCRIPT_DIR/target/$MUSL_TARGET/release/udpcp" "$bin"
}
...
# run_test NAME BIN SIZE_BYTES DELAY_MS LOSS_PCT
run_test() {
